...
class RealESRNet(object):
	...
	def load_srmodel(self, base_dir, model):
		...
		if model is None:
			loadnet = torch.load(os.path.join(self.base_dir, 'gpen', 'realesrnet_x%d.pth' % self.scale))
		else:
			loadnet = torch.load(os.path.join(self.base_dir, 'gpen', model + '_x%d.pth' % self.scale))
