...
class SpecificNorm(nn.Module):
	def __init__(self, epsilon=1e-8):
		"""
            ...
            https://discuss.pyt.org/t/encounter-the-runtimeerror-one-of-the-variables-needed-for-gradient-computation-has-been-modified-by-an-inplace-operation/836/3
        """
		super(SpecificNorm, self).__init__()
		self.mean = np.array([0.485, 0.456, 0.406])
		self.mean = t.from_numpy(self.mean).float().cuda()
		self.mean = self.mean.view([1, 3, 1, 1])

		self.std = np.array([0.229, 0.224, 0.225])
		self.std = t.from_numpy(self.std).float().cuda()
...
class fsModel(BaseModel):
	...
	def initialize(self, opt):
		...
		netArc_checkpoint = opt.Arc_model
		...
		if self.isTrain:
			...
			self.optimizer_G = t.optim.Adam(params, lr=opt.lr, betas=(opt.beta1, 0.999))

			# optimizer D
			params = list(self.netD1.parameters()) + list(self.netD2.parameters())
			self.optimizer_D = t.optim.Adam(params, lr=opt.lr, betas=(opt.beta1, 0.999))

	def _gradinet_penalty_D(self, netD, img_att, img_fake):
		# interpolate sample
		bs = img_fake.shape[0]
		alpha = t.rand(bs, 1, 1, 1).expand_as(img_fake).cuda()
		...
		# compute gradients
		grad = t.autograd.grad(outputs=pred_interpolated, inputs=interpolated, grad_outputs=t.ones(pred_interpolated.size()).cuda(), retain_graph=True, create_graph=True, only_inputs=True)[0]

		# penalize gradients
		grad = grad.view(grad.size(0), -1)
		grad_l2norm = t.sqrt(t.sum(grad**2, dim=1))
		loss_d_gp = t.mean((grad_l2norm - 1)**2)

		return loss_d_gp

	def cosin_metric(self, x1, x2):
		#return np.dot(x1, x2) / (np.linalg.norm(x1) * np.linalg.norm(x2))
		return t.sum(x1 * x2, dim=1) / (t.norm(x1, dim=1) * t.norm(x2, dim=1))
	...
	def forward(self, img_id, img_att, latent_id, latent_att, for_G=False):
		...
		return [self.loss_filter(loss_G_GAN, loss_G_GAN_Feat, loss_G_VGG, loss_G_ID, loss_G_Rec, loss_D_GP, loss_D_real, loss_D_fake), img_fake]
	...
	def update_fixed_params(self):
		...
		self.optimizer_G = t.optim.Adam(params, lr=self.opt.lr, betas=(self.opt.beta1, 0.999))
		if self.opt.verbose:
