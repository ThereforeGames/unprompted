...
#             'retinaface_mnet025_v2')  #can replace with your own face detector
		...
		# self.detector = insightface.app.FaceAnalysis(name='buffalo_l', root=root, providers=['CUDAExecutionProvider'])
		# self.detector.prepare(ctx_id=ctx_id, det_thresh=0.6, det_size=(640,640))
		...
		model = mx.mod.Module(symbol=sym, context=mx.cpu(), label_names=None) # hardcode cpu use for now
